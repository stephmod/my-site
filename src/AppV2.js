...
function AppV2() {
	...
	const work = [
		...
		{
			company: 'Visa',
			roles: ['Staff Design Engineer, Data Experience'],
			year: '2021–2023',
			description:
	...
	return (
		<div className="bg-black text-white min-h-screen overflow-x-clip">
			...
			<section className="min-h-screen flex flex-col lg:flex-row">
				...
				<div className="min-h-[280px] h-[35vh] lg:h-screen lg:flex-1 overflow-visible bg-[#eaeaea]">
						...
						<div key={i} className="grid lg:grid-cols-3 gap-6 lg:gap-12">
							<div>
								...
								{item.roles.map((role, j) => (
									<p key={j} className="font-inter text-gray-400 mt-1">{role}</p>
								))}
								<p className="font-inter text-gray-400 text-sm mt-2">{item.year}</p>
							</div>
